...
rule diamond_NCBI:
    input:
        ...
        db = expand("database/bac_nr_ncbi/bac_nr_tax.dmnd")
    output:
        csv = "{results}/bac_diamond/{sample}_ncbi.csv"
    conda: "../envs/diamond.yaml"
    threads: config["threads"]
    shell:
        """
            diamond blastx -b 20.0 -c 1 -q {input.fasta} -d {input.db} -o {output.csv} --evalue 1 -f 6 qseqid sseqid score evalue pident staxids --max-target-seqs 20 --threads {threads} --unal 1
...
rule blast_ncbi_edit:
    input:
         blast="{results}/bac_diamond/{sample}_ncbi.csv"
    output:
          csv="{results}/bac_diamond/{sample}_ncbi_edit.csv"
    script:
...
rule ncbi_anotation_merge:
    input:
        blast="{results}/bac_diamond/{sample}_ncbi_edit.csv",
