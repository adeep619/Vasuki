...
#sample_df.columns=['Cross-reference (KEGG)' , 'NumReads']
new_df=sample_df[["Cross-reference (KEGG)","NumReads"]].copy()
...
#
sum_df=new_df.groupby(['Cross-reference (KEGG)'])['NumReads'].sum('sum').reset_index()
...
sum_df.columns=["Cross-reference (KEGG)", header]
